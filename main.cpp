...
int main(int argc, char *argv[])
{
    ...
    parser.setApplicationDescription(QStringLiteral("Pop-up with exit options for MX Fluxbox"));
    parser.addOption({{"v", "vertical"}, QStringLiteral("Display buttons in a vertical window")});
    parser.addOption({{"h", "horizontal"}, QStringLiteral("Display buttons in a horizontal window")});
    parser.addOption({QStringLiteral("help"), QStringLiteral("use -v, --vertical to display buttons vertically\nuse -h, --horizontal to display buttons horizontally.")});
    parser.process(a);
    ...
    QTimer::singleShot(5s, &a, &QApplication::quit);
    return a.exec();
}
